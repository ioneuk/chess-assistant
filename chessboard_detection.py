...
def chessboard_detection(input_file_path: str, output_file_path: str):
    img = loadImage(input_file_path)
...
def draw_polygon(image, lines: list, thickness=2):
    lines = list(map(lambda xy: (int(xy[0]), int(xy[1])), lines))
    for i in range(len(lines) - 1):
        image = cv2.line(image, lines[i], lines[i + 1], (255, 0, 0), thickness)
    image = cv2.line(image, lines[len(lines) - 1], lines[0], (255, 0, 0), thickness)
    return image


def pointInsideAPolygon(x: float, y: float, polygon: list):
    point = Point(x, y)
...
# TODO colors
def get_cells(file_path, show: bool = False):
    image = loadImage(file_path)
    M, ideal_grid, grid_next, grid_good, spts = findChessboard(image)

    # View
    if M is not None:
        M, _ = generateNewBestFit((ideal_grid + 8) * 32, grid_next, grid_good)  # generate mapping for warping image
        img_warp = cv2.warpPerspective(image, M, (17 * 32, 17 * 32), flags=cv2.WARP_INVERSE_MAP)

        best_lines_x, best_lines_y = getBestLines(img_warp)

        board_outline_unwarp = getBoardOutline(best_lines_x, best_lines_y, M)
        board_outline_unwarp = board_outline_unwarp.tolist()
        board_outline_unwarp.pop()

        # # Data that might be useful in the future
        # min_x = min(board_outline_unwarp, key=lambda xy: xy[0])[0]
        # max_x = max(board_outline_unwarp, key=lambda xy: xy[0])[0]
        # min_y = min(board_outline_unwarp, key=lambda xy: xy[1])[1]
        # max_y = max(board_outline_unwarp, key=lambda xy: xy[1])[1]
        #
        # cell_width = abs(xy_unwarp[0][0] - xy_unwarp[1][0])
        # cell_height = abs(xy_unwarp[0][1] - xy_unwarp[7][1])

        # Making offset so all points will be in polygon
        offset = 8
        board_outline_unwarp[0][0] -= offset
        board_outline_unwarp[0][1] += offset
        board_outline_unwarp[1][0] += offset
        board_outline_unwarp[1][1] += offset
        board_outline_unwarp[2][0] += offset
        board_outline_unwarp[2][1] -= offset
        board_outline_unwarp[3][0] -= offset
        board_outline_unwarp[3][1] -= offset

        # Filter points that are not in polygon
        cell_points = list(filter(lambda xy: pointInsideAPolygon(xy[0], xy[1], board_outline_unwarp),
                                  grid_next.tolist()))

        if len(cell_points) != 81:
            raise RuntimeError("Not all cell points detected")
        ...
        for i in range(8):
            ...
            for j in range(8):
                cells[i].append([
                    cell_points[9 * i + j],
                    cell_points[9 * i + j + 1],
                    cell_points[9 * (i + 1) + j + 1],
                    cell_points[9 * (i + 1) + j],
                ])
        cells.reverse()

        if show:
            image = draw_polygon(image, board_outline_unwarp)
            for horizontal in cells:
                for cell in horizontal:
                    image = draw_polygon(image, cell)
            for x, y in cell_points[:81]:
...
def get_position(x, y, cells):
    ...
    for i in range(8):
        for j in range(8):
            if pointInsideAPolygon(x, y, cells[i][j]):
                horizontal = i
                vertical = j
                break
        if horizontal != -1:
